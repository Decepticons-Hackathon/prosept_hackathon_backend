...
class JsonResponse(JsonResponseBase):
    ...
    def __init__(self, data, code=status.HTTP_200_OK, message="", *args, **kwargs):
        _ = {"code": code, "message": message, "version": version, "data": data}
...
class CsvParser:
    ...
    def __save_dealer_model(self, line):
        try:
            Dealer.objects.get_or_create(id=force_int(line[0]), name=line[1])
        except Exception as error:
    ...
    def __save_dealer_product_model(self, line):
        try:
            ...
            dict_data = {"product_id": product, "dealer_id": dealer}
            ...
            logger.warning(f"Ошибка добавления данных DealerProduct: {error}")
        return

    def __save_data(self, files):
        if "marketing_dealer" in files:
            with open(files["marketing_dealer"], "r", newline="") as csvfile:
                linereader = csv.reader(csvfile, delimiter=";")
                for row in linereader:
                    if "id" not in row[0]:
                        self.__save_dealer_model(row)
        if "marketing_product" in files:
            with open(files["marketing_product"], "r", newline="") as csvfile:
                linereader = csv.reader(csvfile, delimiter=";")
                for row in linereader:
                    if "id" not in row[1]:
                        self.__save_product_model(row)
        if "marketing_dealerprice" in files:
            with open(files["marketing_dealerprice"], "r", newline="") as csvfile:
                linereader = csv.reader(csvfile, delimiter=";")
                for row in linereader:
                    if "id" not in row[0]:
                        self.__save_dealer_price_model(row)
        if "marketing_productdealerkey" in files:
            with open(files["marketing_productdealerkey"], "r", newline="") as csvfile:
                linereader = csv.reader(csvfile, delimiter=";")
                for row in linereader:
                    if "id" not in row[0]:
